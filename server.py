...
def health():
    return {
        ...
        "s3_configured": False,
        "grok": str(GROK_BIN) if GROK_BIN.exists() else None,
        "devin": str(DEVIN_BIN) if DEVIN_BIN.exists() else None,
        "agy": str(AGY_BIN) if AGY_BIN.exists() else None,
        "pi": str(PI_BIN) if PI_BIN.exists() else None,
        "elicit_cli": ELICIT_CLI.exists(),
        "ffmpeg": shutil.which(FFMPEG) is not None,
        "openrouter_key": pipe.get_openrouter_key() is not None,
        ...
        "hyperagent_session": ha.get_session() is not None,
        "time": utcnow(),
    }
